...
class MultiDisguisedAttackPOEnv(MultiEnv):
...
class MultiAgentDisguisedAttackPOEnv(MultiEnv):
    ...
    * ring_length: bounds on the ranges of ring road lengths the autonomous
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        if any(vel < -100) or kwargs['fail']:
            return {key: -10000 for key in self.k.vehicle.get_rl_ids()}
        ...
        mean_difference = np.mean(np.abs(np.array(idm_actions)-np.array(list(rl_actions.values()))))
        reward -= eta * mean_difference

        result = {key: float(reward) for key in self.k.vehicle.get_rl_ids()}
